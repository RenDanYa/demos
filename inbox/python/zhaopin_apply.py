...
从 Markdown 笔记中提取职位 URL, 调用 opencli zhaopin apply 一键投递。
投递完成后自动复核 (访问 URL 检测按钮状态), 未投递的补投。
直接在原 Markdown 表格中添加"投递状态"列实时更新。

用法:
    python zhaopin_apply.py <md_file>
    python zhaopin_apply.py "d:/obsidian/demo/05_long_project/招聘/智联招聘_录入.md"
    python zhaopin_apply.py <md_file> --start 5      # 从第 5 个开始投递 (断点续传)
    python zhaopin_apply.py <md_file> --delay 3       # 每次投递间隔 3 秒 (默认 5 秒)
    python zhaopin_apply.py <md_file> --dry-run       # 只打印不投递
    python zhaopin_apply.py <md_file> --no-recheck    # 跳过投递后复核
    python zhaopin_apply.py <md_file> --recheck-only # 只复核 (跳过投递, 直接访问 URL 校验并更新状态列)

输出:
    - 终端实时日志
    - 投递记录: d:/obsidian/demo/05_long_project/招聘/投递记录.md
    - 断点续传: 同目录 zhaopin_apply_processed.txt
...
PROCESSED_LOG = OUTPUT_DIR / "zhaopin_apply_processed.txt"
...
def apply_job(url):
    """调用 opencli zhaopin apply 投递单个职位

    返回: (status, message) 或 (None, error)
    """
    args = ["zhaopin", "apply", url, "-f", "json"]
...
def check_job(url):
    """调用 opencli zhaopin check 校验职位是否已投递

    返回: (status, message) 或 (None, error)
        status: applied / not_applied / need_login / verify / offline
    """
    args = ["zhaopin", "check", url, "-f", "json"]
...
"""
    ...
    source = md_path.stem
    row = f"| {now} | {source} | [{job_id}](https://www.zhaopin.com/jobdetail/{job_id}.htm) | {title} | {status} | {message} |\n"
...
"""
    ...
    now = time.strftime('%Y-%m-%d %H:%M:%S')
    row = f"| {now} | [{job_id}](https://www.zhaopin.com/jobdetail/{job_id}.htm) | {title} | {status} | {message} |\n"
...
def update_md_row_status(md_path, job_id, status_text):
    ...
    for i in range(separator_idx + 1, len(lines)):
        ...
        if not line.startswith('|'):
            break
        if job_id not in line or 'zhaopin.com' not in line:
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="智联招聘批量投递职位",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    python zhaopin_apply.py "d:/obsidian/demo/05_long_project/招聘/智联招聘_录入.md"
    python zhaopin_apply.py <md_file> --start 5
    python zhaopin_apply.py <md_file> --delay 3
    python zhaopin_apply.py <md_file> --dry-run
    python zhaopin_apply.py <md_file> --recheck-only
""",
    )
    ...
    parser.add_argument("--recheck-only", action="store_true", help="只复核 (跳过投递阶段, 直接访问 URL 校验并更新状态列)")
    ...
    if args.recheck_only:
        ...
        for i, url in enumerate(urls, 1):
            ...
            if status is None:
                ...
                log("  ⚠ 复核触发腾讯云 EdgeOne 验证, 立即停止复核")
                log(f"  续复核命令: python zhaopin_apply.py \"{md_path}\" --recheck-only --start {i + 1}")
                break
    ...
    for i, url in enumerate(urls, 1):
        ...
        if status is None:
            ...
            append_record(md_path, job_id, "", status, message)
            update_md_row_status(md_path, job_id, "⚠ 风控")
            log("  ⚠ 检测到腾讯云 EdgeOne 验证, 立即停止批量投递")
            log("  请手动访问智联招聘搜索页解除风控后, 使用 --start 参数续投")
            log(f"  续投命令: python zhaopin_apply.py \"{md_path}\" --start {i + 1}")
            break
        else:
