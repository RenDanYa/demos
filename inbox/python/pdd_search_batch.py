...
pdd_search_cheap 的搜索逻辑, 生成多个 Obsidian Markdown 文件。

用法:
    python pdd_search_batch.py                                    # 默认读 未命名 1.md, 每个搜 3 个
...
# 复用 pdd_search_cheap 的搜索/生成逻辑
sys.path.insert(0, str(Path(__file__).parent))
from pdd_search_cheap import (  # noqa: E402
    IMAGES_ROOT,
    OUTPUT_ROOT,
    build_markdown,
    log,
    sanitize_filename,
    search_products_cheap,
    write_markdown,
)
...
def main():
    ...
    for i, keyword in enumerate(items, 1):
        ...
        # 搜索 + 生成 markdown
        try:
            products = search_products_cheap(keyword, limit, images_dir)
            if not products:
                log(f"  失败: 未获取到商品")
